...
# 해싱의 성능 분석
- 이상적인 해싱의 시간 복잡도(충돌이 없는 경우) : `O(1)`
- 적재 밀도(loading density), 적재 비율(loading factor): `저장된 항목의 개수 / 해싱테이블의 버킷의 개수` = `n/M`

|탐색 방법|탐색|삽입|삭제|
|--|--|--|--|
|순차탐색|`O(n)`|`O(1)`|`O(n)`|
|이진탐색|`O(long2n)`|`O(log2n+n)`|`O(log2n+n)`|
|이진탐색트리(균형트리)|`O(log2n)`|`O(log2n)`|`O(log2n)`|
|이진탐색트리(경사트리)|`O(n)`|`O(n)`|`O(n)`|
|해싱(최선)|`O(1)`|`O(1)`|`O(1)`|
|해싱(최악)|`O(n)`|`O(n)`|`O(n)`|
